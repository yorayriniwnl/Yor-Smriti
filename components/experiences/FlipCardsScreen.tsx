...
type SorryCard = {
  id: string;
  front: { emoji: string; title: string; hint: string };
  back: { text: string; sub: string };
};
...
export default function FlipCardsScreen({ cards, flipped, toggleFlip, allFlipped, onReadLetter }: Props) {
  return (
    <div>
      ...
      {allFlipped && (
        ...
        >
          ...
          <button className="btn-primary" onClick={onReadLetter} style={{ width: '100%' }}>
            Read my letter →
          </button>
        </div>
      )}
