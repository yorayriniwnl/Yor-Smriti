...
export function useAmbientSound() {
  ...
  const buildGraph = useCallback((): SoundNodes => {
    ...
    const oscConfigs: Array<{ freq: number; type: OscillatorType; gain: number; detune?: number }> = [
      { freq: 110.00,  type: 'sine',     gain: 0.20 },          // A2
      { freq: 164.81,  type: 'sine',     gain: 0.15 },          // E3
      { freq: 220.00,  type: 'triangle', gain: 0.12 },          // A3
      { freq: 261.63,  type: 'sine',     gain: 0.10 },          // C4
      { freq: 329.63,  type: 'triangle', gain: 0.08 },          // E4
      { freq: 110.00,  type: 'sine',     gain: 0.06, detune: 8 }, // slight detune for warmth
      { freq: 55.00,   type: 'sine',     gain: 0.18 },          // A1 sub
    ];
  ...
  const fadeIn = useCallback((nodes: SoundNodes) => {
    ...
    masterGain.gain.linearRampToValueAtTime(0.28, now + 4.0);
  }, []);
